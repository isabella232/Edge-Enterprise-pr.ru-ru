...
|||
|-|-|
|[Application Guard-Einstellungen](#application-guard-einstellungen)|[Передавать](#cast)|
...
|[Start, Startseite und neue Tabseite](#start-startseite-und-neue-tabseite)|[Дополнительно](#additional)|
...
Как правило, это отключено в качестве защиты от фишинга. If you don't configure this policy, it's disabled and third-party images can't show an authentication prompt.
...
```
  ...
  #### Beschreibung
  Erstellen Sie eine Liste mit URL-Mustern zum Angeben von Websites, die unsichere gemischte Inhalte (d.з) HTTP-Inhalte auf HTTPS-Websites) anzeigen können.
...
```
  ...
  #### Beschreibung
  Erstellen Sie eine Liste mit URL-Mustern zum Angeben von Websites, für die die Anzeige von blockierbaren (d.з) aktiven) gemischten Inhalten (d.з) HTTP-Inhalte auf HTTPS-Websites) nicht zulässig ist und für die Upgrades für optional blockierbare gemischte Inhalte deaktiviert sind.
...
– Legen Sie die Protokolleigenschaft für das Schema fest (z. Б) „mailto“) – Legen Sie die URL-Eigenschaft der Anwendung fest, das im Feld „Protokoll“ angegebenen Schema angegeben wird. Das Muster kann einen „%s“ Platzhalter enthalten, der die behandelte URL ersetzt.
...
Hinweis: Diese Richtlinie steuert Screenshots, die innerhalb des Browsers selbst erstellt wurden. Auch wenn Sie diese Richtlinie aktivieren, können Benutzer möglicherweise weiterhin Screenshots mit einer Methode außerhalb des Browsers (z. Б) mit einem Betriebssystemfeature oder einer anderen Anwendung) aufnehmen.
...
```
  ...
  #### Beschreibung
  Sie können Vorschläge von Vorschlagsanbietern auf dem Gerät (lokale Anbieter) zulassen, z. Б) "Favoriten" und "Browserverlauf", in der Adressleiste und der Liste der automatischen Vorschläge von Microsoft Edge.
...
Die Richtlinie erstellt eine Liste mit Favoriten. Jeder Favorit enthält die Schlüssel "Name" und "URL", die den Namen des Favoriten und sein Ziel enthalten. Sie können einen Unterordner konfigurieren, indem Sie einen Favoriten ohne einen "URL"-Schlüssel definieren, jedoch mit einem zusätzlichen "Children"-Schlüssel, der eine Liste der oben definierten Favoriten enthält (von denen einige möglicherweise wieder Ordner sind). Microsoft Edge ändert unvollständige URLs so, als ob sie über die Adressleiste eingegeben wurden, z. Б) wird "microsoft.com" zu "https://microsoft.com/".
...
```
  ...
  #### Beschreibung
  Gibt eine Liste mit Ursprüngen (URLs) oder Hostnamen-Mustern (z. Б) „*.contoso.com“) an, für die keine Sicherheitseinschränkungen bei unsicheren Ursprüngen gelten.
...
```
  ...
  #### Beschreibung
  Wenn Sie diese Richtlinie aktivieren oder nicht konfigurieren, kann eine Webseite Bildschirmübertragungs-APIs (z.Б) getDisplayMedia() oder die Desktoperfassungserweiterungs-API) für einen Screenshot verwenden.
Wenn Sie diese Richtlinie deaktivieren, schlagen Aufrufe von Bildschirmübertragungs-APIs fehl. Wenn Sie z.Б) eine webbasierte Onlinebesprechung verwenden, funktioniert das Teilen von Videos oder die Bildschirmübertragung nicht.
...
Websites (z. Б) https://contoso.com/some/path) stimmen nur als U2F-appIDs überein. Domänen (z.Б) contoso.com) stimmen nur mit den webauthn-RP-IDs überein. Sie müssen sowohl die appID-URL als auch die Domäne auflisten, um U2F- und webauthn-APIs für eine bestimmte Website zu erfassen.
...
Wenn Sie diese Richtlinie auf "wahr" oder "nicht festgelegt" setzen, werden gemischte Audio- und Videoinhalte automatisch auf HTTPS aktualisiert (d.з) die URL wird als HTTPS ohne Fallback neu geschrieben, wenn die Ressource nicht über HTTPS verfügbar ist), und in der URL-Leiste für gemischte Bildinhalte wird die Warnung "Nicht sicher" angezeigt.
...
Wenn diese Richtlinie aktiviert ist, sendet das Feature “Client Hints des Benutzer-Agenten” präzise Anforderungsheader, die Informationen über den Benutzerbrowser (z.Б) die Browserversion) und die Umgebung (z.Б) die Systemarchitektur) bereitstellen.
...
```
  ...
  #### Beschreibung
  Gibt eine Liste mit Ursprüngen (URLs) oder Hostnamen-Mustern (z. Б) „*contoso.com*“) an, für die lokale IP-Adressen durch WebRTC verfügbar gemacht werden sollen.
