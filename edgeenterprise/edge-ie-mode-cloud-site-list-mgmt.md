...
2. Подписка на клиент должна включать службы Exchange. Дополнительные сведения см. в разделе [Вопросы и ответы](#faq).
3. У администраторов должен быть установлен браузер Microsoft Edge версии 93 или более поздней, а также последняя версия [файлов политик](https://aka.ms/edgeenterprise).
...
### <a name="why-is-my-tenant-required-to-have-an-exchange-service-subscription-for-this-feature"></a>Почему моему клиенту требуется подписка на службу Exchange для этой функции?

Эта подписка обеспечивает немедленную правильную серверную связь с вашим клиентом при первом выборе списков сайтов Microsoft Edge. Если службы Exchange не являются частью вашей подписки, запрос на связь из Центра администрирования Microsoft 365 завершится сбоем. В систему подготовки отправляется оповещение для устранения проблемы.  Это приведет к трехдневной задержке на использование этой возможности.
