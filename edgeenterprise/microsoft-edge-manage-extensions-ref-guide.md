...
Определите, хотите ли вы установить все параметры управления расширением в политике ExtensionSettings или установить эти элементы управления с помощью других политик.
...
Эта политика может управлять такими настройками, как URL-адрес обновления, в котором расширение будет загружено для начальной установки, и заблокированными разрешениями. Вы также можете использовать эту политику, чтобы определить, какие разрешения не разрешены для запуска. Доступные поля политик описаны в следующей таблице.
...
| **override_update_url**| Доступно из Microsoft Edge 93<br>Если это поле задано , Microsoft Edge url-адрес обновления, указанный в политике ExtensionSettings или в политике ExtensionInstallForcelist, для последующих обновлений `true` расширения.<br>Если это поле не установлено или задано, Microsoft Edge использует URL-адрес, указанный в манифесте расширения `false` для обновлений.|
...
В глобальной области разрешены следующие клавиши (*):

- blocked_permissions
- installation_mode - только `"blocked"` `"allowed"` , или являются `"removed"` допустимые значения в этой области.
- runtime_blocked_hosts
...
В отдельной области расширения разрешены следующие клавиши:
...
В области URL-адресов обновления допускаются следующие клавиши:
...
1. Откройте редактор групповой политики и перейдите Microsoft Edge > расширения > настройка политики **настройки расширения.**
2. Введите политику и введите ее компактные данные по нотации объектов JavaScript (JSON) в текстовом окне в виде одной строки без разрывов строки.
...
В предыдущем примере, если вместо "force_installed" используется "normal_installed", расширение автоматически устанавливается без взаимодействия с пользователем, но оно может отключить расширение.  
...
Для параметров, применимых к области по умолчанию (звездочка), используйте следующее расположение в реестре:
