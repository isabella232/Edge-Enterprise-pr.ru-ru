...
Самое простое регулярное выражение — это один буквенный символ. Кроме метасимволов, таких как `\*+?()|`, символы соответствуют самим себе. Чтобы сопоставить метасимволь, экранируйте его обратной косой чертой: `\+` соответствует литералу и символу.
...
Порядок применения операторов: от самого слабого до самого сильного, первое изменение, затем объединение, а в конце — операторы повторения. Явные круглые скобки можно использовать для принудительного использования различных значений, как в арифметических выражениях. Некоторые примеры: _ab|cd_ эквивалентен _(ab)|( cd)_ ; _`ab\`_ эквивалентно _`a(b\)`_ .

Описанный выше синтаксис в основном является стандартным синтаксисом регулярного выражения _egrep_ Unix. Этого подмножества достаточно для описания всех обычных языков. Обычный язык — это набор строк, которые можно сопоставить в одном проходе по тексту, используя только фиксированный объем памяти. Новые средства регулярных выражений (в частности, Perl и скопированные языки) добавили много новых операторов и escape-последовательностей. Эти изменения делают регулярные выражения более краткими, а иногда и более криптографическими, но не более мощными.

На этой странице представлен синтаксис регулярных выражений, принятый в RE2.

Здесь также перечислены некоторые синтаксис, принятый PCRE, PERL и VIM.
